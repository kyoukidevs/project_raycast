...
UpdatingFunctions.ESP = function(Entry, TopTextObject, BottomTextObject)
    ...
    if getrenderproperty(TopTextObject, "Visible") then
        ...
        local Character = IsAPlayer and __index(Entry.Object, "Character") or Entry.Object
        ...
        -- Для кастомных моделей показываем только имя и расстояние
        Content = Settings.DisplayName and Name or ""

        setrenderproperty(TopTextObject, "Text", Content)

        local PlayerPosition = __index(GetModelPrimaryPart(Character), "Position") or Vector3zero
        ...
        setrenderproperty(BottomTextObject, "Text", Content)
    end
end
...
local LoadESP = function()
    -- Загружаем обычных игроков (опционально)
    ...
    -- Отслеживаем удаление кастомных моделей
