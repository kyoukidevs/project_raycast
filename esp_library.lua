--[[
	...
	Modified for Player Models in Folder Support
]]
...
getgenv().ExunysDeveloperESP = {
	DeveloperSettings = {
		...
		RainbowSpeed = 1,
		WidthBoundary = 1.5,
		PlayerModelsFolder = "Players" -- Название папки с моделями игроков
	},
	...
	Settings = {
		...
		EntityESP = true, -- Включить ESP для NPC и моделей
		PlayerModelsESP = true -- Включить ESP для моделей игроков в папке
	},
...
local CoreFunctions = {
	...
	CalculateParameters = function(Object)
		...
		local IsPlayerModel = Object:FindFirstAncestor(Environment.DeveloperSettings.PlayerModelsFolder)
		...
		local RigType = "R15"
		if FindFirstChild(__index(Part, "Parent"), "Torso") then
			RigType = "R6"
		elseif FindFirstChild(__index(Part, "Parent"), "UpperTorso") then
			RigType = "R15"
		end
...
local UtilityFunctions = {
	InitChecks = function(self, Entry)
		if not Entry.IsAPlayer and not Entry.PartHasCharacter and not Entry.RenderDistance and not Entry.IsPlayerModel then
			return
		end
		...
		local IsPlayerModel = Entry.IsPlayerModel
		...
		Entry.Connections.UpdateChecks = Connect(__index(RunService, DeveloperSettings.UpdateMode), function()
			...
			if IsPlayerModel then -- Модель игрока в папке
				Checks.Ready = Settings.PlayerModelsESP and (__index(Player, "Position") - LocalCharacterPosition).Magnitude <= RenderDistance
				return
			end
			...
			if not IsAPlayer then -- NPC
				...
				Checks.Ready = Checks.Ready and Checks.Alive and IsInDistance and Settings.EntityESP
				return
			end
	...
	WrapObject = function(self, Object, PseudoName, Allowed, RenderDistance)
		...
		local IsPlayerModel = Object:FindFirstAncestor(DeveloperSettings.PlayerModelsFolder)
		...
		Entry.PartHasCharacter = not Entry.IsAPlayer and Humanoid and not Entry.IsPlayerModel
	...
	-- Новая функция для отслеживания папки с моделями игроков
	TrackPlayerModelsFolder = function(self)
		local folderName = Environment.DeveloperSettings.PlayerModelsFolder
		local folder = Workspace:FindFirstChild(folderName)
		
		if not folder then
			warn("EXUNYS_ESP > Player models folder '" .. folderName .. "' not found in Workspace")
			return
		end

		-- Обработчик для новых моделей в папке
		local function handleNewModel(model)
			...
			if humanoid and rootPart then
				-- Добавляем ESP для модели
				self:WrapObject(rootPart, model.Name, nil, 500)
			end
		end

		-- Обработчик для удаления моделей
		local function handleRemovedModel(model)
			if not model:IsA("Model") then return end
			self:UnwrapObject(model)
		end
		...
		-- Отслеживаем новые модели
		folder.ChildAdded:Connect(handleNewModel)
		
		-- Отслеживаем удаление моделей
		folder.ChildRemoved:Connect(handleRemovedModel)

		return folder
	end
}

--// Основная функция загрузки ESP
local LoadESP = function()
	-- Загружаем обычных игроков
	...
	-- Загружаем модели игроков из папки
	ServiceConnections.PlayerModelsFolder = UtilityFunctions:TrackPlayerModelsFolder()
end
...
--// Interactive User Functions (остаются без изменений)
...
Environment.UnwrapPlayers = function()
	...
	for _, ConnectionIndex in next, {"PlayerRemoving", "PlayerAdded", "CharacterAdded", "PlayerModelsFolder"} do
...
Environment.SetPlayerModelsFolder = function(folderName)
	Environment.DeveloperSettings.PlayerModelsFolder = folderName
	if Loaded then
		Environment.Restart()
	end
	return true
